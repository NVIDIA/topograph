...
- **Payload:** The request body is a JSON object organized into three top-level sections:
  ...
  - **engine**: (optional) Selects the topology output and provides any engine-specific parameters.
    ...
    - **params**: (optional) A key-value map with engine-specific parameters.
      ...
      - **blockNamePrefixRegexp**: (optional) Used in: [`slurm`, `slinky`]. A Go regular expression that matches a prefix at the start of a node name. For `topology/block`, the matched text becomes the block name; for example, `^srv[0-9]{2}` names the block containing `srv1101` as `srv11`. Blocks whose node names do not match, including empty complemented blocks, retain their default names.
      ...
      - **topologies**: (optional) Used in: [`slurm`, `slinky`]. A map of named per-partition topology settings. Do not set top-level `plugin` together with `topologies`.
        ...
        - **blockSizes**: (optional) Used in: [`slurm`, `slinky`]. An array of block sizes for `topology/block`.
        - **blockNamePrefixRegexp**: (optional) Used in: [`slurm`, `slinky`]. Per-topology form of `blockNamePrefixRegexp`, applied to generated block names in that topology.
