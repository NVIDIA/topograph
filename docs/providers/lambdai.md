...
- **Node Observer trigger** — keep at least one trigger configured. The chart's API readiness watch satisfies this by default; a node selector, pod selector, or positive `periodicInterval` can also do so. To regenerate when Lambda node membership changes, set `nodeObserver.topograph.trigger.nodeSelector` (or `podSelector`) — e.g. `kubernetes.io/os: linux` to watch all nodes. Periodic generation is useful as a fallback for changes that do not produce a watched Kubernetes event.
