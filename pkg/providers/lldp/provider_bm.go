...
func LoaderBM(_ context.Context, providerConfig providers.Config) (providers.Provider, *httperr.Error) {
	...
	if err := validateInterfaces(params.Interfaces); err != nil {
		return nil, httperr.NewError(http.StatusBadRequest, err.Error())
	}

	return &ProviderBM{interfaces: params.Interfaces, run: exec.Pdsh}, nil
}
...
func (p *ProviderBM) GenerateTopologyConfig(ctx context.Context, _ *int, cis []topology.ComputeInstances) (*topology.Graph, *httperr.Error) {
	...
	neighbors, err := parsePdshOutput(stdout, p.interfaces)
	if err != nil {
	...
	if topo.Len() == 0 {
		return nil, httperr.NewError(http.StatusBadGateway, "no LLDP switches discovered; verify lldpd is receiving advertisements and provider.params.interfaces selects the data-plane interface")
	}

	return topo.ToGraph(NAME_BM, cis, 0, false), nil
}

func parsePdshOutput(stdout *bytes.Buffer, interfaces []string) (map[string]Neighbor, error) {
	...
	for scanner.Scan() {
		...
		neighbor, err := selectNeighbor(parsed, interfaces)
