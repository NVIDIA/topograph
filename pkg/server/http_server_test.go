...
// TestTopologyOutputDirFileWrite is an end-to-end test that starts a real HTTP
// server with TopologyOutputDir configured to a temp directory, submits a
// /v1/generate request whose topologyConfigPath is a bare filename, waits for
// the async queue to finish, and asserts the file is physically present in
// TopologyOutputDir with non-empty topology content.
func TestTopologyOutputDirFileWrite(t *testing.T) {
	outputDir := t.TempDir()
	port, err := test.GetAvailablePort()
	require.NoError(t, err)
	...
	// Request uses a bare filename — the server must resolve it to
	// TopologyOutputDir/topology.conf before the engine writes the file.
	payload := `{
		"provider": {"name": "test"},
		"engine": {
			"name": "slurm",
	...
	// Poll until the async worker has finished.
	deadline := time.Now().Add(10 * time.Second)
	var status int
	for time.Now().Before(deadline) {
		r, e := http.Get(fmt.Sprintf("%s/v1/topology?uid=%s", baseURL, uid))
		require.NoError(t, e)
	...
	require.Equal(t, http.StatusOK, status, "request did not complete in time")

	// The file must be inside TopologyOutputDir, not in the working directory.
	want := filepath.Join(outputDir, "topology.conf")
	data, err := os.ReadFile(want)
	require.NoError(t, err, "topology.conf was not written to TopologyOutputDir")
	require.NotEmpty(t, data, "topology.conf is empty")

	// Double-check the file did NOT land in the working directory.
	_, cwdErr := os.Stat("topology.conf")
	require.True(t, os.IsNotExist(cwdErr),
		"topology.conf must not be created in the working directory")
}

// TestTopologyOutputDirResolution verifies two behaviours of the server-level
// topologyConfigPath validation when TopologyOutputDir is configured:
//  1. A bare filename is resolved to TopologyOutputDir/filename in the engine
//     params so the file actually lands in the configured directory.
//  2. A path outside TopologyOutputDir is rejected with HTTP 400.
func TestTopologyOutputDirResolution(t *testing.T) {
	dir := t.TempDir()
	srv = &HttpServer{
		cfg: &config.Config{
			Provider:          "test",
			Engine:            "slurm",
			TopologyOutputDir: dir,
		},
	}

	t.Run("bare filename is resolved to TopologyOutputDir", func(t *testing.T) {
		payload := `{
			"provider": {"name": "test"},
			"engine":   {"name": "slurm", "params": {"topologyConfigPath": "topology.conf"}}
		}`
		r := &http.Request{
			Method: http.MethodPost,
			Body:   io.NopCloser(bytes.NewBuffer([]byte(payload))),
		...
		req := readRequest(w, r)
		require.NotNil(t, req)
		got, _ := req.Engine.Params["topologyConfigPath"].(string)
		require.Equal(t, dir+"/topology.conf", got,
			"bare filename must be resolved to TopologyOutputDir/filename")
	})

	t.Run("path within TopologyOutputDir is accepted unchanged", func(t *testing.T) {
		fullPath := dir + "/sub/topology.conf"
		payload := fmt.Sprintf(`{
			"provider": {"name": "test"},
			"engine":   {"name": "slurm", "params": {"topologyConfigPath": %q}}
		}`, fullPath)
		r := &http.Request{
			Method: http.MethodPost,
			Body:   io.NopCloser(bytes.NewBuffer([]byte(payload))),
	...
	t.Run("path outside TopologyOutputDir is rejected", func(t *testing.T) {
		payload := `{
			"provider": {"name": "test"},
			"engine":   {"name": "slurm", "params": {"topologyConfigPath": "/etc/cron.d/malicious"}}
		}`
		r := &http.Request{
			Method: http.MethodPost,
			Body:   io.NopCloser(bytes.NewBuffer([]byte(payload))),
